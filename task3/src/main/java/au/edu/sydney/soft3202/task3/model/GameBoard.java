...
public class GameBoard {
    ...
    public List<String> getUserSaveNames() {
        return db.getUserSaveNames(this.userId);
    }

    public String saveGame(String name) {
        String serialise = serialise();
        String error = db.addSave(name, serialise, this.userId);
        return error;
    }

    public String overrideSave(String name) {
        String serialise = serialise();
        String error = db.updateState(name, serialise, this.userId);
        return error;
    }

    public String loadGame(String name) {
        List<String> state = db.getState(name, userId);
        if (state.size() > 0) {
            if (state.get(0) == null) {
                return state.get(1);
            }
            else {
                deserialise(state.get(0));
            }
        }
        return null;
    }
